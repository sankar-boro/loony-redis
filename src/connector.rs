...
use loony::codec::{AsyncRead, AsyncWrite};
use loony::connect::{self, Address, Connect, Connector};
use loony::framed::{ReadTask, State, WriteTask};
use loony::{service::Service, time::Seconds, util::ByteString};

#[cfg(feature = "openssl")]
use loony::connect::openssl::{OpensslConnector, SslConnector};

#[cfg(feature = "rustls")]
use loony::connect::rustls::{ClientConfig, RustlsConnector};
    ...
    T::Response: AsyncRead + AsyncWrite + Unpin + 'static,
{
    ...
    pub fn connect(&self) -> impl Future<Output = Result<Client, ConnectError>> {
        ...
        async move {
            ...
            loony::rt::spawn(ReadTask::new(io.clone(), state.clone()));
            loony::rt::spawn(WriteTask::new(io, state.clone()));
